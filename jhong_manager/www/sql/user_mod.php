<?php 
	...
	if ( isset ( $_REQUEST['action'] )) {
		if ( $_REQUEST['action'] == "yes" ) {
			...
			$note      = $_REQUEST['note'];
                        ...
                        // 先防呆！因為 email 不可以重複！
                        $checksql = "select count(email) from j_user where email = '$email' and username != '$username' ";
                        $checkres = mysql_query($checksql,$link);
                        $checkrow = mysql_fetch_row($checkres);
                        if ( $checkrow[0] >= 1 ) {
                                echo "此 email '$email' 已經存在系統中 $checkrow[0] 個了！無法修改！";
                                echo "<script>alert('更新失敗！email 重複');location.href='$web/sql/user_list.php';</script>";
                                die;
                        }
			...
			$upresult = mysql_query($upsql,$link);
	...
	$theuid = $_REQUEST['uid'];
	$sql = "select username,all_name,level,email,u_group,tel,note,jointime from j_user where uid = $theuid ";
...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">網頁帳號資訊觀察與管理</h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <form  class="form-horizontal" method="post" action="user_mod.php" OnSubmit="return checkme(this)">
